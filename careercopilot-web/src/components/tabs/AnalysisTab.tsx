...
const getProductionUrl = (url: string) => {
  ...
  // If the backend returns a localhost link, replace it with the actual Render URL
...
export function ApplyTab() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Open the cleaned production URL
      window.open(getProductionUrl(res.download_url), "_blank")
    } catch (e: any) {
          ...
          <div className="space-y-4">
            ...
            {result && (
              <>
                ...
                {/* Generate resume section with FIXED URL logic */}
                <div className="rounded-xl p-5" style={{ background: 'rgba(0,229,160,0.04)', border: '1px solid rgba(0,229,160,0.15)' }}>
                  ...
                  <p className="text-sm mb-3" style={{ color: 'var(--text-muted)' }}>Generate a PDF resume tailored to this exact job description.</p>
                  ...
                  {generateResult ? (
                    <div className="flex items-center gap-3">
                      ...
                      <a href={getProductionUrl(generateResult.download_url)} target="_blank" rel="noopener noreferrer"
